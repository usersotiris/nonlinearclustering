...
# dd = input dataset, sample are in rows and variables in columns
# k = number of clusters, optional argument
# knei = the number of neighbors used for building the knn-graph, optional argument
# SP = parameter specifying the splitting methodology, Values are "DEN" and "MAR" for idivclu-D and idivclu-M respectivelly. 
# adjust = bandwidth multiplier for the kernel density estimation, optional argument
# interpolation_points = interpolation points for the kernel density estimation, optional argument
# min_outlier = minimum number of points constituting a cluster, optional argument

# Outputs

# Clusterids = clustering output, assigned class label for each sample
# AllRes = description of the hierarchical structure, sample id assigned at each node of the resulting tree
# leafs = information for resulting leafs of the tree
...
ProcessClust<-function(x,cids,adjust,min_outlier,interpolation_points,SP){
  ...
  if (SP == "DEN"){
	    if(length(projections) > 2) {
          d<-density( projections, bw="SJ" ,adjust=adjust, n=min(length(projections), interpolation_points) )
          ord_mid <- sr_projections[-length(sr_projections)] + diff(sr_projections)/2
          newd <- approx(d$x,d$y,xout=ord_mid)
          d$x <- newd$x
          d$y <- newd$y
      } else {
          d<-density( projections, adjust=adjust , n=min(length(projections), interpolation_points))
      }
...
idivclu<-function(dd,k,adjust=0.5, min_outlier=round(max((dim(dd)[1]/k)/3,5)), interpolation_points = (dim(dd)[1]/2), knei=5,SP="DEN"){ 
  ...
  cdata <- cal_dist_mat(dd,knei)
	...
	# return( list(  Clusterids=Clusters, AllRes=cres,  leafs=leafs, max_depth=max_depth) ) ;
	return( list(  Clusterids=Clusters, AllRes=cres,  leafs=leafs) ) ;
}
