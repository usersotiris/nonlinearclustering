...
cal_dist_mat <- function(dd,kapa){
  ...
  if (un_com$no>1){
    ...
    # populate the original distance matrix with new connections
...
# =============================================================================
ProcessClust<-function(x,cids,adjust,min_outlier,interpolation_points) {
...
idivclu<-function(dd,k,adjust=0.5, min_outlier=max((dim(dd)[1]/k)/3,5), interpolation_points = dim(dd)[1], knei=5){ 
  print(sprintf("min_outlier:%f",min_outlier))
  
  # ------------------------
  # buld the geodesic distance matrix
  ...
  # ------------------------
	...
	cres<-vector("list" , 1)
	max_depth<-1;
	cres[[1]]<-list(node = ProcessClust(cdata,ids,adjust,min_outlier,interpolation_points) );
	...
	while( noClusters<k &  nrow(SplitPointsVals) > 0 ) {
		...
		kid1[[1]]<- list(node = ProcessClust(cdata,  cres[[ Cluster.To.Split ]]$node$kid1.ids  ,adjust, min_outlier, interpolation_points) );
		kid2[[1]]<- list(node = ProcessClust(cdata,  cres[[ Cluster.To.Split ]]$node$kid2.ids  ,adjust, min_outlier, interpolation_points) );
